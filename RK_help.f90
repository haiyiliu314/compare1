  ...
  do i1 = 1,N_sec
    freq_bas(i1) =1d0/dble(N_freq)*(i1 - 0.5d0)-0.5d0
...
!  freqgrid = (Eg - 4d0*E_B +freqgrid*6d0*E_B)/hbar
...
!    Etime = E_excit*exp(-t1*t1/(sigmat*sigmat)) * (1d0 + exp(-ii*2.0d0*omega_1s*t1))!no E_B   RWA
    Etime = A_excit*1.0d3*exp(-t1*t1/(sigmat_A*sigmat_A))*dcos(A_freq_para*t1) !unit: mV/m
  end function Etime
