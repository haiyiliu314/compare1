...
!-----------output the frequency grid---------------
  freqgrid = (0.5d0*Eg+freqgrid*0.15d0*Eg)/hbar
!  freqgrid = (Eg - 4d0*Ebind +freqgrid*6d0*Ebind)/hbar
...
!------------end output-----------------------------
  ...
  !function for calculating electrical field(unit: binding energy)
    ...
    Etime = A_excit*1.0d3*exp(-t1*t1/(sigmat_A*sigmat_A)) * (A_freq_para*(exp(ii*delta_1s*t1)*exp(ii*2.0d0*omega_1s*t1)-exp(-ii*delta_1s*t1))/(2.0d0*ii) + 2.0d0*t1/(sigmat_A*sigmat_A) * (exp(-ii*delta_1s*t1) + exp(ii*delta_1s*t1)*exp(ii*2.0d0*omega_1s*t1))/2.0d0)!meV non RWA, 1d3 for eV to meV 
...
!    Etime = E_excit*exp(-t1*t1/(sigmat*sigmat)) * (1d0 + exp(-ii*2.0d0*omega_1s*t1))!no Ebind   RWA
  end function Etime
...
!      coup = 0d0
      ...
      p_out(Ndo_m, :) = -ii*(y*y*p_via(Ndo_m, :) - 2.0d0*pf_sum - 250.0d0/hbar*1d-7*y*Atime(nt_via)*coup&
                        +shift*p_via(Ndo_m, :)&
...
!      write(807, format_V) -real((conjg(Etime(nt_via)*dipole/Ebind)*p_via(Nm_o+1, :) - Etime(nt_via)*dipole/Ebind&
!                        *conjg(p_via(Nm_o+1, :)))&
!                        /hbar*dt*ii*Ebind)
!      write(808, format_V) real(f_out(Nm_o+1,:))
    ...
    kP1freq_out = dt * (dy*sum(y*y*(p_via(Nm_o, :)+p_via(Nm_o+2, :)))/(2.0d0*pi)) * exp(ii*freqgrid*t1)
  end subroutine RHS
  end module RK_help
