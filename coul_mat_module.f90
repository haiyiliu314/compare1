...
!a:integer series   b&w11: Gauss-Chebyshev grid
    integer                                          ::Ndo, Ndo_in, Ndo_in_in, Ndo_m, m = 0, Ndo_sec   
...
!help terms to help calculating Gauss-Chebyshev grid
  ...
  open(unit=700,file=list_file)
    ...
    do Ndo_m = 1, Nm_o+1
      do Ndo_sec = 1, N_sec
        do Ndo = 1, N_fine
          fine_grid(Ndo) = (dble(Ndo)*dy_fine_sec(Ndo_sec)  -0.50d0*dy_fine_sec(Ndo_sec)  )
          !fine_grid: finer grid for singularity removal
        end do

        do Ndo = (Ny1_help(Ndo_sec)+1), (Ny1_help(Ndo_sec+1))
          y_fine = y(Ndo) - dy_sec(Ndo_sec)/2.0d0 + fine_grid   
...
!------------calculate the Coulomb matrix without removal of singularity-------------
          do Ndo_in = 1, Nphi
            ...
            coul_mat(Ndo_m, :, Ndo)=coul_mat(Ndo_m, :, Ndo)+(1.0d0/y_leng)*2.0d0*y*w11(Ndo_in)*dcos(dble(Ndo_m-1)*b(Ndo_in))/pi*dy1
            coul_mat_cc(Ndo_m, :, Ndo)=coul_mat_cc(Ndo_m, :, Ndo)+(1.0d0/y_leng)*2.0d0*y*w11(Ndo_in)*dcos(dble(Ndo_m-1)*b(Ndo_in))/pi*dy1*gcc_conf   
            coul_mat_ch(Ndo_m, :, Ndo)=coul_mat_ch(Ndo_m, :, Ndo)+(1.0d0/y_leng)*2.0d0*y*w11(Ndo_in)*dcos(dble(Ndo_m-1)*b(Ndo_in))/pi*dy1*gch_conf   
            coul_mat_hh(Ndo_m, :, Ndo)=coul_mat_hh(Ndo_m, :, Ndo)+(1.0d0/y_leng)*2.0d0*y*w11(Ndo_in)*dcos(dble(Ndo_m-1)*b(Ndo_in))/pi*dy1*ghh_conf   
          end do
...
!-----------calculate the diagonal terms with removal of singularity---------------------
          do Ndo_in = 1, N_fine 
            ...
            do Ndo_in_in = 1,Nphi
              ...
              round_y1 = int(y_leng1*(N_read-1)/y_para(N_read))
              ...
              coul_mat(Ndo_m, Ndo, Ndo)=coul_mat(Ndo_m, Ndo, Ndo) +(1.0d0/y_leng1 * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos(dble(Ndo_m-1)*b(Ndo_in_in) ))/pi*dy_sec(Ndo_sec)
              coul_mat_cc(Ndo_m, Ndo, Ndo)=coul_mat_cc(Ndo_m, Ndo, Ndo) + (1.0d0/y_leng1 * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos(dble(Ndo_m-1)*b(Ndo_in_in) ))/pi*dy_sec(Ndo_sec)*gcc_conf1
              coul_mat_ch(Ndo_m, Ndo, Ndo)=coul_mat_ch(Ndo_m, Ndo, Ndo) + (1.0d0/y_leng1 * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos(dble(Ndo_m-1)*b(Ndo_in_in) ))/pi*dy_sec(Ndo_sec)*gch_conf1
              coul_mat_hh(Ndo_m, Ndo, Ndo)=coul_mat_hh(Ndo_m, Ndo, Ndo) + (1.0d0/y_leng1 * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos(dble(Ndo_m-1)*b(Ndo_in_in) ))/pi*dy_sec(Ndo_sec)*ghh_conf1
!            write(format_V, '(A12, I4, A18)')   '(SE24.16e3, ', Ny, '(", ",SE24.16e3))'
!            write(700, format_V) y_leng1
            end do
!          write(*,*) y_fine(Ndo_in)
          end do  
          coul_mat(Ndo_m, Ndo, Ndo) = coul_mat(Ndo_m, Ndo, Ndo)/dble(N_fine)
          coul_mat_cc(Ndo_m, Ndo, Ndo) = coul_mat_cc(Ndo_m, Ndo, Ndo)/dble(N_fine)
          coul_mat_ch(Ndo_m, Ndo, Ndo) = coul_mat_ch(Ndo_m, Ndo, Ndo)/dble(N_fine)
          coul_mat_hh(Ndo_m, Ndo, Ndo) = coul_mat_hh(Ndo_m, Ndo, Ndo)/dble(N_fine)
!-----------end calculation-----------------------------------------------------------
        end do
      end do
    end do
    close(700)
      ...
      coul_mat_f = (coul_mat_cc+coul_mat_hh)/2.0d0
!coul_mat_f = 0.0d0
