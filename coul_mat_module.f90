...
!------------calculate the Coulomb matrix without removal of singularity-------------
        do Ndo_in = 1, Nphi
          ...
          round_y = int(y_leng*N_read/y_para(N_read)) !Nread: the number of elements in y_para.txt
          do Ndo_in_in = 1,Ny
...
!-----------calculate the diagonal terms with removal of singularity---------------------
        do Ndo_in = 1, N_fine 
          do Ndo_in_in = 1,Nphi
            ...
            coul_mat(Ndo_m, Ndo,Ndo)=coul_mat(Ndo_m, Ndo, Ndo) + (y_leng1/ dble(N_fine) * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos( dble(Ndo_m-1)*b(Ndo_in_in) ))
            coul_mat_cc(Ndo_m, Ndo, Ndo)=coul_mat_cc(Ndo_m, Ndo, Ndo) + (y_leng1/ dble(N_fine) * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos( dble(Ndo_m-1)*b(Ndo_in_in) ))*gcc_conf1
            coul_mat_ch(Ndo_m, Ndo, Ndo)=coul_mat_ch(Ndo_m, Ndo, Ndo) + (y_leng1/ dble(N_fine) * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos( dble(Ndo_m-1)*b(Ndo_in_in) ))*gch_conf1
            coul_mat_hh(Ndo_m, Ndo, Ndo)=coul_mat_hh(Ndo_m, Ndo, Ndo) + (y_leng1/ dble(N_fine) * 2.0d0 *y_fine(Ndo_in) * w11(Ndo_in_in)* dcos( dble(Ndo_m-1)*b(Ndo_in_in) ))*ghh_conf1
