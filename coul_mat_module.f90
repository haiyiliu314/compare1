...
module coul_mat_module
  ...
  !calculate Coulomb matrix(non-symmetric, i.e. already contains the integration term, i.e. 2pi*dy*y*V_{y-y'})
    implicit none
    real*8                                 ::a(Nphi), b(Nphi), c(Nphi), w11(Nphi)   
...
!Ndo & Ndo_in:integer number for do loop, m:order of circular harmonics 
    real*8                                 ::fine_grid(N_fine)
!fine_grid: finer grid for singularity removal
    real*8                                 ::b_mid1, b_mid2(Nphi), b_mid3(Nphi)  
!help terms to help calculating Gauss-Chebyshev grid
    ...
    do Ndo = 1, N_fine
      fine_grid(Ndo) = (real(Ndo,8)-0.50d0)*dy_fine  
      !fine_grid: finer grid for singularity removal
    end do
    b_mid1 = ( real(Nphi,8) + 1.0d0 ) / (2.0d0 * pi )
    b_mid2 = sin ( (2.0d0 * pi * a ) / ( real(Nphi,8)+ 1.0d0 ))
    b_mid3 =  ( a - b_mid1 * b_mid2)
    b = pi / ( real(Nphi,8) + 1.0d0 ) * b_mid3
    w11 = pi / ( real(Nphi,8) + 1.0d0 ) * ( 1.0d0 - cos( 2.0d0 * pi * a / ( real(Nphi,8) + 1.0d0))) 
...
!------------calculate the Coulomb matrix without removal of singularity-------------
        do Ndo_in = 1, Nphi
          coul_mat(Ndo_m, :, Ndo)=coul_mat(Ndo_m, :, Ndo)+(1.0d0/(sqrt(y(Ndo)*y(Ndo)+y*y -2.0d0*y*y(Ndo)*cos(b(Ndo_in)))))*2.0d0*y*w11(Ndo_in)*cos(real(Ndo_m-1,8)*b(Ndo_in))
        end do
!-----------end calculation----------------------------------------------------------
...
!-----------calculate the diagonal terms with removal of singularity---------------------
        do Ndo_in = 1, N_fine    
          coul_mat(Ndo_m, Ndo,Ndo)=coul_mat(Ndo_m, Ndo, Ndo) + sum( 1.0d0/ sqrt( y(Ndo) *y(Ndo) + y_fine(Ndo_in) *y_fine(Ndo_in) - 2.0d0 * y_fine(Ndo_in) * y(Ndo) * cos( b ) ) / real(N_fine,8) * 2.0d0 *y_fine(Ndo_in) * w11* cos( real(Ndo_m-1,8)*b ))
        end do  
!-----------end calculation-----------------------------------------------------------
      end do
    end do
