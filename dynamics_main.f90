...
!------------------initialization-------------------------------------------------------------
  A_freq_para = (Eg-abs(W2(1))*Ebind)/hbar/3d0
  A_excit = Ascale/A_freq_para*1d8
...
!        write(808, format_V) aimag(decay(i1, :))
      ...
      i3 = i3+1
      write(700, format_V)  -abs(f(Nm_o+1,:))+abs(f(Nm_o+1,:))*abs(f(Nm_o+1,:))+abs(p(Nm_o+1,:))*abs(p(Nm_o+1,:)) 
!-abs(f(Nm_o+1,:))+abs(f(Nm_o+1,:))*abs(f(Nm_o+1,:))+abs(p(Nm_o+1,:))*abs(p(Nm_o+1,:)) 
    end if
  end do
  close(700)
  close(701)
...
!-------------------output--------------------
  ...
  open(unit=700,file=list_file)
  write(format_V, '(A12, I4, A18)')   '(SE24.16e3, ', Ny, '(", ",SE24.16e3))'
      write(700, format_V) abs(VR(:,1))
  close(700)
  write(list_file, '(A)') 'eigenvec_2p.dat'       !eigen values
  open(unit=700,file=list_file)
  write(format_V, '(A12, I4, A18)')   '(SE24.16e3, ', Ny, '(", ",SE24.16e3))'
      write(700, format_V) abs(VR1(:,1))
  close(700)
  write(list_file, '(A)') 'eigenvec_3d.dat'       !eigen values
  open(unit=700,file=list_file)
  write(format_V, '(A12, I4, A18)')   '(SE24.16e3, ', Ny, '(", ",SE24.16e3))'
      write(700, format_V) abs(VR2(:,1))
  close(700)
  write(list_file, '(A)') 'eigenvec_s.dat'       !eigen values
  open(unit=700,file=list_file)
  write(format_V, '(A12, I4, A18)')   '(SE24.16e3, ', Ny, '(", ",SE24.16e3))'
  do i1 = 1, Ny
      write(700, format_V) abs(VR(:,i1))
  end do
  close(700)
