...
for N_t = 1:Nt
    for i = 1:(i1(2)-1)
        ...
        peak_f(i,:, N_t) = max(fk((Nm_o*N_t-Nm_o+1):(Nm_o*N_t),range)');
    end
end

fs_0 = abs(fk(Nm_o*(1:Nt)-Nm,1));
...
title(['s state, dt = ',num2str(params(2),3), ' as, dy = ',num2str(dk), ', Emax = ', num2str(params(6),3), 'MV/cm, $\gamma$ = ', num2str(params(7)), 'meV, run',num2str(num_run)],'FontName','Arial','FontSize',nfont1)
xlabel('t(ps)','FontName','Arial','FontSize',nfont2)
ylabel('$f_s$, peak','FontName','Arial','FontSize',nfont2)
...
xlabel('t(ps)','FontName','Arial','FontSize',nfont2)
ylabel('$|p_s|$, peak','FontName','Arial','FontSize',nfont2)

h=gcf;
% set(h, 'PaperOrientationMode', 'auto');
% set(h, 'PaperOrientation', 'landscape');
print(h, '-dpdf', '-fillpage', ['/home/liuhai/dynamics/Cir_Harm_Exp/multi_photon/run',num2str(num_run), '/nfig_peak_s'])