...
!-------------------end LAPACK-----------------------
  ...
  double precision                             ::ymax = 10.0d0, dy = 0d0, gamma = 0.2d0/Ebind
  ...
  double precision                             ::freqgrid(N_freq) = 0.0d0, test(Ny) = 0.0d0, readin = 0.0d0    
...
!    A_excit = 39.810717055349780/A_freq_para*1d8

    decay_m = 5.0d0*gamma;

    open(unit = 100, file = 'fort.42', status = 'old', action = 'read')
    read(100,*) readin
!    A_freq_para = (Eg-(0.5d0 - 0.02d0*readin)*Ebind)/hbar/2d0


!    Nt = 5d3*scale1*(21d0-readin)
!    A_excit = (1d0*readin)*1d8/A_freq_para
...
!    scale1 = 2.0d0*hbar/(gamma*Ebind)
    ...
    A_excit = 1d-1*(1d3)**(1/10d0*readin-1d0)/A_freq_para*1d8
!    A_excit = 0.4d0*0.05d0/sigmat_A/A_freq_para*1d8		!fix E*tau
    dt = t_end/dble(Nt)  
