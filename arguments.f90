...
!                                                 sigmat_A = 0.2d0*scale1, tstart_A = -0.5d0*scale1  
  ...
  double precision, parameter                  ::omega_1s = (0d0)/hbar, &    
                                                 ...
                                                 A_freq_para = (Eg-(4d0/9d0)*Ebind)/2d0/hbar
...
!-------------------end LAPACK-----------------------
  double precision                             ::E_excit = 1.0d-3, shift = (Eg - omega_1s*hbar)/Ebind,A_excit = 1d5*(Eg/3d0/hbar)/A_freq_para, gamma_m = 1d0
  !E_excit: excitation level(unit: binding energy)  shift: shift caused by rotation frame 
  !(unit: binding energy)
  !A_excit: unit: V*ps/m
  !gamma_m: dephasing caused by quantum memory unit: meV
