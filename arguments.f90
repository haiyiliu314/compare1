...
!  !m: order of circular harmonics
  double precision, parameter                  ::ymax = 15.0d0, dy = ymax/dble(Ny) 
...
!                                                 sigmat_A = 0.2d0*scale1, tstart_A = -0.5d0*scale1  
  ...
  double precision, parameter                  ::Ebind = 4.18d0, gamma = 1d0/Ebind, Eg = 1.50d3, decay_m =5.0d0*gamma
...
!-------------------end LAPACK-----------------------
  double precision                             ::E_excit = 1.0d-3, shift = (Eg - omega_1s*hbar)/Ebind, A_excit = 0.0d0, A_freq_para = 0.0d0, Ascale = 0.5d0
  ...
  double precision                             ::freqgrid(N_freq) = 0.0d0, test(Ny) = 0.0d0     
