...
!Runge-Kutta for: FFT of electrical field & polarization; time evolution of polarization and density
  ...
  implicit none
  contains
  subroutine RK(E_in, Pfreq_in, P1freq_in, f_in, p_in, n, A_in, J_THZ_in, &
                E_out, Pfreq_out, P1freq_out, f_out, p_out, A_out, J_THZ_out)
  !07/03/2017 creation
  !Runga_Kutta subroutine
    ...
    complex*16, dimension(N_freq)                       ::kPfreq1, kPfreq2, kPfreq3, kPfreq4,& 
                                                          kP1freq1, kP1freq2, kP1freq3, kP1freq4
    ...
    call RHS(n, f_in, p_in, &
             kp1, kf1, kE1, kPfreq1, kP1freq1, kA1, kJ1)

    !calculate k2
    call RHS(n+0.50d0, f_in + kf1/2.0d0, p_in + kp1/2.0d0, &
             kp2, kf2, kE2, kPfreq2, kP1freq2, kA2, kJ2)

    !calculate k3
    call RHS(n+0.50d0, f_in + kf2/2.0d0, p_in + kp2/2.0d0, &
             kp3, kf3, kE3, kPfreq3, kP1freq3, kA3, kJ3)

    !calculate k4
    call RHS(n+1.0d0, f_in + kf3, p_in + kp3, &
             kp4, kf4, kE4, kPfreq4, kP1freq4, kA4, kJ4)
    ...
    P1freq_out = P1freq_in + kP1freq1/6.0d0 +kP1freq2/3.0d0 + kP1freq3/3.0d0 + kP1freq4/6.0d0
    end subroutine RK
